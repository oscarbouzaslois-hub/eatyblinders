...
/* Persistencia local del dispositivo (gratis, sin backend) */
const __store = {
...
const RECETAS = [
    ...
    { id: "lentejas", n: "Lentejas con verduras", cat: "comida", dif: 2, min: 50, rac: 4,
        ...
        e: "🍲", nut: { kcal: 380, p: 20, cb: 55, g: 8 }, tags: ["alta en fibra", "de cuchara", "batch cooking"],
    ...
    { id: "crema_calabacin", n: "Crema de calabacín", cat: "cena", dif: 1, min: 30, rac: 4,
        ...
        e: "🥣", nut: { kcal: 150, p: 5, cb: 12, g: 9 }, tags: ["vegetariana", "ligera", "apta para niños"],
...
function likesInForm(pref, forma) {
    if (!pref || pref.like === undefined)
        return null; // sin valorar
    const formas = pref.formas || {};
    if (forma && formas[forma] !== undefined)
        return formas[forma];
    return pref.like;
}
...
function matchReceta(receta, prefs) {
    ...
    for (const it of receta.ing) {
        ...
        if (v === true) {
            ...
            noGusta++;
            conflictos.push(it.id);
        }
    }
    if (valorables === 0)
        return { score: null, conflictos, aciertos, valorables };
...
function agregarCompra(items) {
    ...
    for (const it of items) {
        if (!porIng[it.ingId])
            porIng[it.ingId] = { sums: {}, libres: [], recetas: new Set() };
        const e = porIng[it.ingId];
        e.recetas.add(it.recetaId);
        const p = parseQty(it.q);
        if (p)
            e.sums[p.unit] = (e.sums[p.unit] || 0) + p.num;
        else if (it.q)
            e.libres.push(it.q);
    }
    const out = [];
...
function IngImg({ ing, size = 96, round = 24 }) {
    ...
    const cat = CAT[ing.c];
    if (ext >= 2 || !slug) {
        return (_jsx("div", { "aria-hidden": true, style: { width: size, height: size, borderRadius: round, flexShrink: 0,
                background: `linear-gradient(145deg, ${cat.soft}, #ffffff 90%)`,
                border: `1.5px solid ${cat.color}22`, display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx("div", { style: { width: size * 0.72, height: size * 0.72, borderRadius: "50%", background: "#fff",
                    boxShadow: `0 3px 10px ${cat.color}33, inset 0 0 0 2px ${cat.color}22`,
                    display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.4 }, children: ing.e }) }));
    }
    return (_jsx("div", { style: { width: size, height: size, borderRadius: round, flexShrink: 0, overflow: "hidden",
            background: `linear-gradient(145deg, ${cat.soft}, #fff)`, border: `1.5px solid ${cat.color}22`,
            display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx("img", { src: `https://img.spoonacular.com/ingredients_250x250/${src}`, alt: ing.n, onError: () => setExt(e => e + 1), loading: "lazy", style: { width: "82%", height: "82%", objectFit: "contain", mixBlendMode: "multiply" } }) }));
}
function Chip({ children, color = T.sub, bg = "#F1EEE8", style }) {
    return _jsx("span", { style: { display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 600,
            color, background: bg, borderRadius: 999, padding: "4px 10px", whiteSpace: "nowrap", ...style }, children: children });
}
function BtnPrimario({ children, onClick, style, disabled }) {
    return _jsx("button", { onClick: onClick, disabled: disabled, style: {
            ...
            justifyContent: "center", gap: 8, boxShadow: disabled ? "none" : "0 6px 16px rgba(232,83,44,0.35)", ...style
        }, children: children });
}
/* ---------------- DESCUBRIR: la cata de mercado ---------------- */
function orderDeck(prefs) {
    return INGS.filter(i => !prefs[i.id] || prefs[i.id].like === undefined)
        .map((i, idx) => ({ i, k: (idx * 2654435761) % 233280 }))
        .sort((a, b) => a.k - b.k).map(x => x.i);
}
function Descubrir({ st, setSt, onOpenFormas }) {
    ...
    const decidir = useCallback((like) => {
        if (!ing)
            return;
        setSt(s => ({ ...s, prefs: { ...s.prefs, [ing.id]: { like, formas: s.prefs[ing.id]?.formas || {} } },
            historial: [...(s.historial || []), ing.id].slice(-30) }));
        setDrag(null);
        if (ing.f && ing.f.length)
            onOpenFormas(ing, like);
    }, [ing, setSt, onOpenFormas]);
    ...
    const onDown = (e) => { if (!ing)
        return; start.current = { x: e.clientX, y: e.clientY }; e.currentTarget.setPointerCapture?.(e.pointerId); };
    const onMove = (e) => { if (!start.current)
        return; setDrag({ x: e.clientX - start.current.x, y: e.clientY - start.current.y }); };
    const onUp = () => {
        if (drag && Math.abs(drag.x) > 90)
            decidir(drag.x > 0);
        else
            setDrag(null);
        start.current = null;
    ...
    return (_jsxs("div", { style: { padding: "8px 20px 20px", display: "flex", flexDirection: "column", height: "100%" }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }, children: [_jsx("h1", { className: "yt-display", style: { fontSize: 26, fontWeight: 800, margin: 0 }, children: "La cata" }), _jsxs("span", { style: { fontSize: 13, fontWeight: 600, color: T.sub }, children: [valorados, "/", total, " catados"] })] }), _jsx("div", { style: { height: 6, borderRadius: 3, background: "#E9E5DD", marginBottom: 14 }, children: _jsx("div", { style: { height: 6, borderRadius: 3, width: `${(valorados / total) * 100}%`, background: T.brand, transition: "width .3s" } }) }), !ing ? (_jsxs("div", { className: "yt-pop", style: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", gap: 10 }, children: [_jsx("div", { style: { fontSize: 56 }, children: "\uD83C\uDFC6" }), _jsx("h2", { className: "yt-display", style: { margin: 0, fontSize: 22 }, children: "\u00A1Mercado completo!" }), _jsxs("p", { style: { color: T.sub, margin: 0, maxWidth: 260 }, children: ["Has valorado los ", total, " ingredientes. Ajusta lo que quieras desde tu perfil."] })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { style: { flex: 1, position: "relative", minHeight: 340 }, children: [deck[1] && (_jsx("div", { style: { position: "absolute", inset: 0, transform: "scale(0.95) translateY(12px)", opacity: 0.5 }, children: _jsx(CartaIngrediente, { ing: deck[1] }) })), _jsx("div", { role: "group", "aria-label": `Ingrediente: ${ing.n}`, onPointerDown: onDown, onPointerMove: onMove, onPointerUp: onUp, onPointerCancel: onUp, style: { position: "absolute", inset: 0, touchAction: "none", cursor: "grab",
                                    transform: `translate(${dx}px, ${(drag?.y || 0) * 0.15}px) rotate(${dx / 22}deg)`,
                                    transition: drag ? "none" : "transform .25s ease" }, children: _jsx(CartaIngrediente, { ing: ing, veredicto: dx > 40 ? "si" : dx < -40 ? "no" : null }) })] }), _jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 22, paddingTop: 16 }, children: [_jsx("button", { "aria-label": "No me gusta", onClick: () => decidir(false), style: { width: 62, height: 62, borderRadius: "50%", background: "#fff", boxShadow: T.shadow, display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx(X, { size: 28, color: T.bad, strokeWidth: 2.6 }) }), _jsx("button", { "aria-label": "Deshacer", onClick: deshacer, disabled: !(st.historial || []).length, style: { width: 46, height: 46, borderRadius: "50%", background: "#fff", boxShadow: T.shadow, display: "flex", alignItems: "center", justifyContent: "center", opacity: (st.historial || []).length ? 1 : 0.35 }, children: _jsx(RotateCcw, { size: 20, color: T.sub }) }), _jsx("button", { "aria-label": "Me gusta", onClick: () => decidir(true), style: { width: 62, height: 62, borderRadius: "50%", background: T.ok, boxShadow: "0 8px 20px rgba(31,157,99,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx(Heart, { size: 28, color: "#fff", strokeWidth: 2.4, fill: "#fff" }) })] }), cat && _jsx("p", { style: { textAlign: "center", fontSize: 12.5, color: T.sub, margin: "12px 0 0" }, children: "Desliza a la derecha si te gusta \u00B7 a la izquierda si no" })] }))] }));
}
function CartaIngrediente({ ing, veredicto }) {
    const cat = CAT[ing.c];
    return (_jsxs("div", { style: { height: "100%", background: T.card, borderRadius: 28, boxShadow: T.shadow, overflow: "hidden",
            display: "flex", flexDirection: "column", border: `1px solid ${T.line}`, position: "relative", userSelect: "none" }, children: [_jsxs("div", { style: { background: `linear-gradient(160deg, ${cat.soft} 0%, #ffffff 85%)`, flex: 1,
                    display: "flex", alignItems: "center", justifyContent: "center", position: "relative", minHeight: 0 }, children: [_jsx("div", { style: { position: "absolute", top: 14, left: 14 }, children: _jsx(Chip, { color: cat.color, bg: "#ffffffE6", children: cat.label }) }), veredicto === "si" && _jsx("div", { className: "yt-fade", style: { position: "absolute", top: 18, right: 18, border: `3px solid ${T.ok}`, color: T.ok, fontWeight: 800, borderRadius: 12, padding: "4px 12px", transform: "rotate(8deg)", fontSize: 18, background: "#fff" }, children: "ME GUSTA" }), veredicto === "no" && _jsx("div", { className: "yt-fade", style: { position: "absolute", top: 18, right: 18, border: `3px solid ${T.bad}`, color: T.bad, fontWeight: 800, borderRadius: 12, padding: "4px 12px", transform: "rotate(-8deg)", fontSize: 18, background: "#fff" }, children: "PASO" }), _jsx(IngImg, { ing: ing, size: 190, round: 36 })] }), _jsxs("div", { style: { padding: "16px 20px 18px" }, children: [_jsx("h2", { className: "yt-display", style: { margin: "0 0 8px", fontSize: 24, fontWeight: 800 }, children: ing.n }), _jsxs("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" }, children: [_jsxs(Chip, { children: [_jsx(Flame, { size: 13 }), " ", ing.kcal, " kcal/100g"] }), ing.p >= 8 && _jsxs(Chip, { color: T.ok, bg: "#E4F5EC", children: ["Prote\u00EDna ", ing.p, "g"] }), ing.f && _jsxs(Chip, { color: T.brand, bg: T.brandSoft, children: ["\u2726 ", ing.f.length, " formas de tomarlo"] })] })] })] }));
}
...
function FormasSheet({ data, onClose, onSave }) {
    ...
    return (_jsxs(Sheet, { onClose: onClose, title: like ? `¿De qué formas te gusta ${ing.n.toLowerCase()}?` : `¿Lo salvamos de alguna forma?`, children: [_jsx("p", { style: { margin: "0 0 14px", color: T.sub, fontSize: 14 }, children: like ? "Desmarca las que no vayan contigo." : `Has dicho que ${ing.n.toLowerCase()} no te va… pero quizá sí de alguna manera concreta:` }), _jsx("div", { style: { display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }, children: ing.f.map(f => (_jsxs("button", { onClick: () => toggle(f), style: {
                    ...
                    }, children: [_jsx("span", { children: f }), sel[f] ? _jsx(Check, { size: 18, color: T.ok }) : _jsx(X, { size: 18, color: "#B7B2A9" })] }, f))) }), _jsx(BtnPrimario, { onClick: () => onSave(ing.id, sel), children: "Guardar matices" }), _jsx("button", { onClick: onClose, style: { width: "100%", padding: 14, color: T.sub, fontWeight: 600, fontSize: 14 }, children: "Omitir" })] }));
}
function Sheet({ children, onClose, title }) {
    return (_jsx("div", { className: "yt-fade", onClick: onClose, style: { position: "absolute", inset: 0, background: "rgba(20,16,28,0.45)", zIndex: 50, display: "flex", alignItems: "flex-end" }, children: _jsxs("div", { className: "yt-up yt-scroll", onClick: e => e.stopPropagation(), role: "dialog", "aria-label": title, style: { background: T.card, width: "100%", borderRadius: "26px 26px 0 0", padding: "10px 20px 24px", maxHeight: "85%", overflowY: "auto" }, children: [_jsx("div", { style: { width: 40, height: 4, borderRadius: 2, background: "#DDD8CF", margin: "6px auto 14px" } }), title && _jsx("h3", { className: "yt-display", style: { margin: "0 0 10px", fontSize: 19, fontWeight: 800 }, children: title }), children] }) }));
}
/* ---------------- RECETAS ---------------- */
function TileReceta({ r, size = 74 }) {
    return (_jsx("div", { "aria-hidden": true, style: { width: size, height: size, borderRadius: 18, flexShrink: 0,
            background: "linear-gradient(150deg, #FCEAE3, #FFF6EE 70%)", border: "1.5px solid #F2D9CD",
            display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx("div", { style: { width: size * 0.7, height: size * 0.7, borderRadius: "50%", background: "#fff",
                boxShadow: "0 3px 10px rgba(232,83,44,0.22)", display: "flex", alignItems: "center",
                justifyContent: "center", fontSize: size * 0.38 }, children: r.e }) }));
}
function badgeMatch(score) {
    if (score === null)
        return { txt: "Cata ingredientes para ver tu match", bg: "#F1EEE8", col: T.sub };
    if (score >= 80)
        return { txt: `${score}% contigo`, bg: "#E4F5EC", col: "#14603D" };
    if (score >= 50)
        return { txt: `${score}% contigo`, bg: "#FBF3DC", col: "#8A6A14" };
...
function Recetas({ st, setSt, abrir, sorprender }) {
    ...
    const lista = useMemo(() => {
        ...
        if (q.trim()) {
            ...
            l = l.filter(({ r }) => r.n.toLowerCase().includes(t) || r.tags.some(x => x.includes(t)));
        }
        if (fDif)
            l = l.filter(({ r }) => r.dif === fDif);
        if (fTiempo)
            l = l.filter(({ r }) => r.min <= fTiempo);
        if (fCat !== "todas")
            l = l.filter(({ r }) => r.cat === fCat);
        if (soloFavs)
            l = l.filter(({ r }) => st.favs.includes(r.id));
    ...
    return (_jsxs("div", { style: { padding: "8px 0 20px", height: "100%", display: "flex", flexDirection: "column" }, children: [_jsxs("div", { style: { padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [_jsx("h1", { className: "yt-display", style: { fontSize: 26, fontWeight: 800, margin: 0 }, children: "Recetas" }), _jsxs("button", { onClick: sorprender, style: { display: "flex", alignItems: "center", gap: 6, background: T.brandSoft, color: T.brand, fontWeight: 700, fontSize: 13.5, borderRadius: 999, padding: "9px 14px" }, children: [_jsx(Dices, { size: 16 }), " Sorpr\u00E9ndeme"] })] }), _jsx("div", { style: { padding: "12px 20px 8px" }, children: _jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 14, padding: "10px 14px" }, children: [_jsx(Search, { size: 17, color: T.sub }), _jsx("input", { value: q, onChange: e => setQ(e.target.value), placeholder: "Busca por nombre o etiqueta\u2026", style: { border: "none", outline: "none", flex: 1, fontSize: 14.5, background: "transparent", color: T.ink } }), q && _jsx("button", { onClick: () => setQ(""), "aria-label": "Borrar b\u00FAsqueda", children: _jsx(X, { size: 16, color: T.sub }) })] }) }), _jsxs("div", { className: "yt-scroll", style: { display: "flex", gap: 8, overflowX: "auto", padding: "4px 20px 10px" }, children: [_jsx("button", { style: selChip(soloFavs), onClick: () => setSoloFavs(v => !v), children: "\u2665 Favoritas" }), [["todas", "Todo"], ["desayuno", "Desayuno"], ["comida", "Comida"], ["cena", "Cena"], ["postre", "Postre"], ["aperitivo", "Aperitivo"]].map(([v, l]) => _jsx("button", { style: selChip(fCat === v), onClick: () => setFCat(v), children: l }, v)), [1, 2, 3].map(d => _jsx("button", { style: selChip(fDif === d), onClick: () => setFDif(x => x === d ? 0 : d), children: DIF[d].l }, d)), [[20, "≤ 20 min"], [35, "≤ 35 min"]].map(([v, l]) => _jsx("button", { style: selChip(fTiempo === v), onClick: () => setFTiempo(x => x === v ? 0 : v), children: l }, v))] }), _jsxs("div", { className: "yt-scroll", style: { flex: 1, overflowY: "auto", padding: "2px 20px 10px", display: "flex", flexDirection: "column", gap: 10 }, children: [lista.length === 0 && (_jsxs("div", { style: { textAlign: "center", color: T.sub, padding: "40px 20px" }, children: [_jsx("div", { style: { fontSize: 40, marginBottom: 8 }, children: "\uD83C\uDF7D\uFE0F" }), "Ninguna receta encaja con esos filtros. Prueba a quitar alguno."] })), lista.map(({ r, m }) => {
                        const b = badgeMatch(m.score);
                        const probada = st.probadas[r.id];
                        return (_jsxs("button", { onClick: () => abrir(r.id), className: "yt-fade", style: { display: "flex", gap: 12, alignItems: "center", background: "#fff", borderRadius: T.radius, border: `1px solid ${T.line}`, padding: 12, textAlign: "left", boxShadow: "0 2px 8px rgba(36,31,46,0.04)" }, children: [_jsx(TileReceta, { r: r }), _jsxs("div", { style: { flex: 1, minWidth: 0 }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 6 }, children: [_jsx("span", { className: "yt-display", style: { fontWeight: 700, fontSize: 15.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: r.n }), st.favs.includes(r.id) && _jsx(Heart, { size: 14, color: T.bad, fill: T.bad, style: { flexShrink: 0 } }), probada && _jsxs(Chip, { color: "#14603D", bg: "#E4F5EC", style: { flexShrink: 0, padding: "2px 8px" }, children: [_jsx(Check, { size: 11 }), " Probada"] })] }), _jsxs("div", { style: { display: "flex", gap: 10, color: T.sub, fontSize: 12.5, margin: "4px 0 6px", fontWeight: 500 }, children: [_jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 3 }, children: [_jsx(Clock, { size: 12 }), " ", r.min, " min"] }), _jsx("span", { style: { color: DIF[r.dif].c, fontWeight: 700 }, children: DIF[r.dif].l }), _jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 3 }, children: [_jsx(Flame, { size: 12 }), " ", r.nut.kcal, " kcal/raci\u00F3n"] })] }), _jsx(Chip, { color: b.col, bg: b.bg, style: { padding: "3px 9px", fontSize: 11.5 }, children: b.txt }), m.conflictos.length > 0 && m.score !== null &&
                                            _jsxs(Chip, { color: "#9C2B3D", bg: "#FBE6EA", style: { padding: "3px 9px", fontSize: 11.5, marginLeft: 4 }, children: ["lleva ", ING_MAP[m.conflictos[0]].n.toLowerCase(), m.conflictos.length > 1 ? ` +${m.conflictos.length - 1}` : ""] })] }), _jsx(ChevronRight, { size: 18, color: "#C9C4BB", style: { flexShrink: 0 } })] }, r.id));
                    })] })] }));
}
/* ---------------- DETALLE DE RECETA ---------------- */
function RecetaDetalle({ recId, st, setSt, cerrar, cocinar, addCompra, addPlan }) {
    ...
    return (_jsxs("div", { className: "yt-fade", style: { position: "absolute", inset: 0, background: T.bg, zIndex: 40, display: "flex", flexDirection: "column" }, children: [_jsxs("div", { className: "yt-scroll", style: { flex: 1, overflowY: "auto" }, children: [_jsxs("div", { style: { background: "linear-gradient(160deg, #FCEAE3, #FFF8F2 80%)", padding: "14px 20px 22px", position: "relative" }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 14 }, children: [_jsx("button", { onClick: cerrar, "aria-label": "Volver", style: { width: 40, height: 40, borderRadius: 12, background: "#fff", boxShadow: T.shadow, display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx(ChevronLeft, { size: 20 }) }), _jsx("button", { onClick: toggleFav, "aria-label": fav ? "Quitar de favoritas" : "Guardar en favoritas", style: { width: 40, height: 40, borderRadius: 12, background: "#fff", boxShadow: T.shadow, display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx(Heart, { size: 20, color: T.bad, fill: fav ? T.bad : "none" }) })] }), _jsxs("div", { style: { display: "flex", gap: 14, alignItems: "center" }, children: [_jsx(TileReceta, { r: r, size: 92 }), _jsxs("div", { children: [_jsx("h1", { className: "yt-display", style: { margin: "0 0 6px", fontSize: 22, fontWeight: 800, lineHeight: 1.15 }, children: r.n }), _jsxs("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" }, children: [_jsx(Chip, { color: b.col, bg: b.bg, children: b.txt }), probada && _jsxs(Chip, { color: "#14603D", bg: "#E4F5EC", children: [_jsx(Check, { size: 12 }), " Probada"] })] })] })] }), _jsx("p", { style: { color: "#7A5C4E", margin: "12px 0 0", fontSize: 14 }, children: r.desc })] }), _jsxs("div", { style: { padding: "16px 20px 130px", display: "flex", flexDirection: "column", gap: 18 }, children: [_jsxs("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" }, children: [_jsxs(Chip, { children: [_jsx(Clock, { size: 13 }), " ", r.min, " min"] }), _jsx(Chip, { color: DIF[r.dif].c, bg: "#fff", style: { border: `1.5px solid ${DIF[r.dif].c}44` }, children: DIF[r.dif].l }), _jsxs(Chip, { children: [_jsx(Utensils, { size: 13 }), " ", r.rac, " raciones"] }), r.tags.map(t => _jsx(Chip, { children: t }, t))] }), _jsxs("section", { children: [_jsx("h2", { className: "yt-display", style: { fontSize: 16, fontWeight: 800, margin: "0 0 8px" }, children: "Por raci\u00F3n" }), _jsx("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }, children: [["kcal", r.nut.kcal, "🔥"], ["proteína", r.nut.p + " g", "💪"], ["carbohid.", r.nut.cb + " g", "🌾"], ["grasas", r.nut.g + " g", "🥑"]].map(([l, v, ic]) => (_jsxs("div", { style: { background: "#fff", borderRadius: 16, border: `1px solid ${T.line}`, padding: "10px 6px", textAlign: "center" }, children: [_jsx("div", { style: { fontSize: 15 }, children: ic }), _jsx("div", { className: "yt-display", style: { fontWeight: 800, fontSize: 15 }, children: v }), _jsx("div", { style: { fontSize: 10.5, color: T.sub, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.4 }, children: l })] }, l))) })] }), _jsxs("section", { children: [_jsx("h2", { className: "yt-display", style: { fontSize: 16, fontWeight: 800, margin: "0 0 8px" }, children: "Ingredientes" }), _jsx("div", { style: { background: "#fff", borderRadius: T.radius, border: `1px solid ${T.line}`, overflow: "hidden" }, children: r.ing.map((it, i) => {
                                            ...
                                            return (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderTop: i ? `1px solid ${T.line}` : "none" }, children: [_jsx(IngImg, { ing: ing, size: 40, round: 12 }), _jsxs("div", { style: { flex: 1, minWidth: 0 }, children: [_jsx("div", { style: { fontWeight: 600, fontSize: 14 }, children: ing.n }), _jsxs("div", { style: { fontSize: 12.5, color: T.sub }, children: [it.q, it.forma ? ` · ${it.forma}` : ""] })] }), v === true && st2 === "con_matices" && it.forma && _jsx(Chip, { color: "#14603D", bg: "#E4F5EC", style: { fontSize: 11 }, children: "en esta forma, s\u00ED \u2713" }), v === true && !(st2 === "con_matices" && it.forma) && _jsx("span", { title: "Te gusta", style: { width: 10, height: 10, borderRadius: 5, background: T.ok, flexShrink: 0 } }), v === false && _jsx(Chip, { color: "#9C2B3D", bg: "#FBE6EA", style: { fontSize: 11 }, children: "no te va" }), v === null && _jsx("span", { title: "Sin valorar", style: { width: 10, height: 10, borderRadius: 5, background: "#D8D4CC", flexShrink: 0 } })] }, i));
                                        }) })] }), _jsxs("section", { children: [_jsx("h2", { className: "yt-display", style: { fontSize: 16, fontWeight: 800, margin: "0 0 8px" }, children: "Pasos" }), _jsx("div", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: r.pasos.map((p, i) => (_jsxs("div", { style: { display: "flex", gap: 10, background: "#fff", borderRadius: 16, border: `1px solid ${T.line}`, padding: "12px 14px" }, children: [_jsx("div", { className: "yt-display", style: { fontWeight: 800, color: T.brand, minWidth: 22 }, children: i + 1 }), _jsxs("div", { style: { fontSize: 14, lineHeight: 1.45 }, children: [p.t, p.s && _jsxs(Chip, { color: T.brand, bg: T.brandSoft, style: { marginLeft: 6, fontSize: 11 }, children: [_jsx(Timer, { size: 11 }), " ", Math.round(p.s / 60) || 1, " min"] })] })] }, i))) })] }), _jsxs("section", { children: [_jsx("h2", { className: "yt-display", style: { fontSize: 16, fontWeight: 800, margin: "0 0 8px" }, children: "\u00BFLa has probado?" }), _jsxs("div", { style: { background: "#fff", borderRadius: T.radius, border: `1px solid ${T.line}`, padding: 14, display: "flex", alignItems: "center", gap: 8, justifyContent: "space-between" }, children: [_jsx("div", { style: { display: "flex", gap: 4 }, children: [1, 2, 3, 4, 5].map(n => (_jsx("button", { "aria-label": `${n} estrellas`, onClick: () => setRating(n), children: _jsx(Star, { size: 26, color: "#E8B93B", fill: probada && probada.rating >= n ? "#E8B93B" : "none" }) }, n))) }), probada
                                                ? _jsx("button", { onClick: quitarProbada, style: { fontSize: 12.5, color: T.sub, fontWeight: 600 }, children: "Quitar" })
                                                : _jsx("span", { style: { fontSize: 12.5, color: T.sub }, children: "Punt\u00FAa para marcarla" })] })] }), _jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }, children: [_jsxs("button", { onClick: () => addCompra(r), style: { background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 16, padding: "13px 10px", fontWeight: 700, fontSize: 13.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }, children: [_jsx(ShoppingBasket, { size: 17, color: T.brand }), " A la compra"] }), _jsxs("button", { onClick: () => addPlan(r.id), style: { background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 16, padding: "13px 10px", fontWeight: 700, fontSize: 13.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }, children: [_jsx(CalendarDays, { size: 17, color: T.brand }), " Al plan"] })] })] })] }), _jsx("div", { style: { position: "absolute", left: 0, right: 0, bottom: 0, padding: "12px 20px 18px", background: "linear-gradient(transparent, #F5F3EE 40%)" }, children: _jsxs(BtnPrimario, { onClick: () => cocinar(r.id), children: [_jsx(ChefHat, { size: 19 }), " Modo cocina"] }) })] }));
}
...
function beep() {
    try {
        ...
        [0, 0.35, 0.7].forEach(t0 => {
            ...
            o.stop(ctx.currentTime + t0 + 0.32);
        });
    }
    catch (e) { /* sin audio */ }
}
function ModoCocina({ recId, cerrar, marcarProbada }) {
    ...
    return (_jsxs("div", { style: { position: "absolute", inset: 0, background: T.ink, color: "#fff", zIndex: 60, display: "flex", flexDirection: "column" }, className: "yt-fade", children: [_jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px" }, children: [_jsxs("button", { onClick: cerrar, "aria-label": "Salir del modo cocina", style: { color: "#fff", display: "flex", alignItems: "center", gap: 6, fontWeight: 600, fontSize: 14, opacity: 0.85 }, children: [_jsx(X, { size: 18 }), " Salir"] }), _jsx("span", { className: "yt-display", style: { fontWeight: 700, fontSize: 14, opacity: 0.85 }, children: r.n })] }), _jsx("div", { style: { display: "flex", gap: 4, padding: "0 20px 8px" }, children: r.pasos.map((_, i) => _jsx("div", { style: { flex: 1, height: 4, borderRadius: 2, background: i < paso || fin ? T.brand : i === paso ? "#fff" : "#ffffff33" } }, i)) }), fin ? (_jsxs("div", { className: "yt-pop", style: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, textAlign: "center", gap: 12 }, children: [_jsx("div", { style: { fontSize: 64 }, children: "\uD83C\uDF89" }), _jsx("h2", { className: "yt-display", style: { margin: 0, fontSize: 26, fontWeight: 800 }, children: "\u00A1Al plato!" }), _jsxs("p", { style: { opacity: 0.75, margin: 0 }, children: ["Has terminado ", r.n.toLowerCase(), ". \u00BFLa marcamos como probada?"] }), _jsx("div", { style: { display: "flex", gap: 8, marginTop: 10 }, children: [1, 2, 3, 4, 5].map(n => (_jsx("button", { "aria-label": `${n} estrellas`, onClick: () => { marcarProbada(r.id, n); cerrar(); }, children: _jsx(Star, { size: 34, color: "#E8B93B", fill: "none" }) }, n))) }), _jsx("button", { onClick: cerrar, style: { color: "#fff", opacity: 0.7, fontWeight: 600, marginTop: 8, fontSize: 14 }, children: "Ahora no" })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { style: { flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 26px", gap: 22 }, children: [_jsxs("span", { className: "yt-display", style: { color: T.brand, fontWeight: 800, fontSize: 14, letterSpacing: 1 }, children: ["PASO ", paso + 1, " DE ", r.pasos.length] }), _jsx("p", { className: "yt-display", style: { fontSize: 23, lineHeight: 1.4, fontWeight: 600, margin: 0 }, children: p.t }), p.s != null && (_jsxs("div", { style: { background: "#ffffff12", borderRadius: 22, padding: 18, display: "flex", alignItems: "center", gap: 16 }, children: [_jsx("div", { className: "yt-display", style: { fontSize: 42, fontWeight: 800, fontVariantNumeric: "tabular-nums", color: resta === 0 ? T.brand : "#fff" }, children: mmss(Math.max(0, resta ?? p.s)) }), _jsxs("div", { style: { display: "flex", gap: 8, marginLeft: "auto" }, children: [_jsx("button", { "aria-label": activo ? "Pausar" : "Iniciar temporizador", onClick: () => setActivo(a => !a), style: { width: 54, height: 54, borderRadius: "50%", background: T.brand, display: "flex", alignItems: "center", justifyContent: "center" }, children: activo ? _jsx(Pause, { size: 22, color: "#fff" }) : _jsx(Play, { size: 22, color: "#fff", style: { marginLeft: 3 } }) }), _jsx("button", { "aria-label": "Reiniciar", onClick: () => { setResta(p.s); setActivo(false); }, style: { width: 54, height: 54, borderRadius: "50%", background: "#ffffff22", display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx(RotateCcw, { size: 20, color: "#fff" }) })] })] })), resta === 0 && _jsx("p", { style: { color: T.brand, fontWeight: 700, margin: 0 }, children: "\u23F0 \u00A1Tiempo! Pasa al siguiente paso." })] }), _jsxs("div", { style: { display: "flex", gap: 10, padding: "16px 20px 24px" }, children: [_jsxs("button", { onClick: () => setPaso(x => Math.max(0, x - 1)), disabled: paso === 0, style: { flex: 1, padding: 15, borderRadius: 16, background: "#ffffff18", color: "#fff", fontWeight: 700, opacity: paso === 0 ? 0.4 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }, children: [_jsx(ChevronLeft, { size: 18 }), " Anterior"] }), _jsxs("button", { onClick: () => setPaso(x => x + 1), style: { flex: 1.4, padding: 15, borderRadius: 16, background: T.brand, color: "#fff", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, boxShadow: "0 6px 16px rgba(232,83,44,0.45)" }, children: [paso === r.pasos.length - 1 ? "Terminar" : "Siguiente", " ", _jsx(ChevronRight, { size: 18 })] })] })] }))] }));
}
/* ---------------- PLAN SEMANAL ---------------- */
function Plan({ st, setSt, abrirReceta, addSemanaCompra }) {
    ...
    const candidatas = useMemo(() => {
        ...
        if (q.trim()) {
            const t = q.trim().toLowerCase();
            l = l.filter(({ r }) => r.n.toLowerCase().includes(t));
        }
        return l;
    }, [picker, q, st.prefs]);
    const nSlots = Object.keys(st.plan).length;
    return (_jsxs("div", { style: { padding: "8px 20px 20px", height: "100%", display: "flex", flexDirection: "column" }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }, children: [_jsx("h1", { className: "yt-display", style: { fontSize: 26, fontWeight: 800, margin: 0 }, children: "Tu semana" }), nSlots > 0 && _jsxs("button", { onClick: addSemanaCompra, style: { display: "flex", alignItems: "center", gap: 6, background: T.brandSoft, color: T.brand, fontWeight: 700, fontSize: 13, borderRadius: 999, padding: "9px 13px" }, children: [_jsx(ShoppingBasket, { size: 15 }), " Semana a la compra"] })] }), _jsxs("p", { style: { color: T.sub, fontSize: 13.5, margin: "0 0 12px" }, children: ["Toca un hueco para elegir plato. ", nSlots ? `${nSlots} de 14 planificados.` : ""] }), _jsx("div", { className: "yt-scroll", style: { flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, paddingBottom: 10 }, children: DIAS.map((d, di) => {
                    const kcal = COMIDAS_PLAN.reduce((acc, c) => {
                        const rec = RECETAS.find(r => r.id === st.plan[`${di}-${c.id}`]);
                        return acc + (rec ? rec.nut.kcal : 0);
                    }, 0);
                    return (_jsxs("div", { style: { background: "#fff", borderRadius: T.radius, border: `1px solid ${T.line}`, padding: 12 }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }, children: [_jsx("span", { className: "yt-display", style: { fontWeight: 800, fontSize: 15 }, children: d }), kcal > 0 && _jsxs("span", { style: { fontSize: 12, color: T.sub, fontWeight: 600 }, children: [kcal, " kcal planificadas"] })] }), _jsx("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }, children: COMIDAS_PLAN.map(c => {
                                    const key = `${di}-${c.id}`;
                                    const rec = RECETAS.find(r => r.id === st.plan[key]);
                                    return rec ? (_jsxs("div", { style: { background: T.brandSoft, borderRadius: 14, padding: "9px 10px", position: "relative" }, children: [_jsx("div", { style: { fontSize: 10.5, fontWeight: 700, color: T.brand, textTransform: "uppercase", letterSpacing: 0.5 }, children: c.l }), _jsxs("button", { onClick: () => abrirReceta(rec.id), style: { fontWeight: 700, fontSize: 12.5, textAlign: "left", lineHeight: 1.25, marginTop: 2, color: T.ink }, children: [rec.e, " ", rec.n] }), _jsx("button", { "aria-label": `Quitar ${rec.n}`, onClick: () => quitarSlot(key), style: { position: "absolute", top: 6, right: 6 }, children: _jsx(X, { size: 13, color: "#B08575" }) })] }, c.id)) : (_jsxs("button", { onClick: () => { setPicker({ dia: di, comida: c.id }); setQ(""); }, style: { border: `1.5px dashed ${T.line}`, borderRadius: 14, padding: "12px 10px", color: T.sub, fontSize: 12.5, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, background: "#FBFAF7" }, children: [_jsx(Plus, { size: 14 }), " ", c.l] }, c.id));
                                }) })] }, d));
                }) }), picker && (_jsxs(Sheet, { onClose: () => setPicker(null), title: `${COMIDAS_PLAN.find(c => c.id === picker.comida).l} del ${DIAS[picker.dia].toLowerCase()}`, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, background: "#F6F3EE", borderRadius: 12, padding: "9px 12px", marginBottom: 10 }, children: [_jsx(Search, { size: 16, color: T.sub }), _jsx("input", { autoFocus: true, value: q, onChange: e => setQ(e.target.value), placeholder: "Buscar receta\u2026", style: { border: "none", outline: "none", flex: 1, fontSize: 14, background: "transparent" } })] }), _jsx("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: candidatas.slice(0, 30).map(({ r, m }) => {
                            const b = badgeMatch(m.score);
                            return (_jsxs("button", { onClick: () => { setSlot(`${picker.dia}-${picker.comida}`, r.id); setPicker(null); }, style: { display: "flex", alignItems: "center", gap: 10, padding: "9px 6px", borderRadius: 12, textAlign: "left" }, children: [_jsx("span", { style: { fontSize: 22 }, children: r.e }), _jsxs("div", { style: { flex: 1, minWidth: 0 }, children: [_jsx("div", { style: { fontWeight: 700, fontSize: 14 }, children: r.n }), _jsxs("div", { style: { fontSize: 12, color: T.sub }, children: [r.min, " min \u00B7 ", DIF[r.dif].l, " \u00B7 ", r.nut.kcal, " kcal"] })] }), m.score !== null && _jsxs(Chip, { color: b.col, bg: b.bg, style: { fontSize: 11 }, children: [m.score, "%"] })] }, r.id));
                        }) })] }))] }));
}
/* ---------------- LISTA DE LA COMPRA ---------------- */
function Compra({ st, setSt }) {
    ...
    const porCat = useMemo(() => {
        ...
        for (const g of grupos) {
            const c = ING_MAP[g.ingId]?.c || "condimento";
            (m[c] = m[c] || []).push(g);
        }
        return m;
    }, [grupos]);
    ...
    return (_jsxs("div", { style: { padding: "8px 20px 20px", height: "100%", display: "flex", flexDirection: "column" }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" }, children: [_jsx("h1", { className: "yt-display", style: { fontSize: 26, fontWeight: 800, margin: 0 }, children: "La compra" }), total > 0 && _jsxs("span", { style: { fontSize: 13, fontWeight: 700, color: T.sub }, children: [hechos, "/", total, " en el carro"] })] }), _jsx("p", { style: { color: T.sub, fontSize: 13.5, margin: "2px 0 12px" }, children: "Se genera sola desde tus recetas y tu plan semanal." }), _jsxs("div", { style: { display: "flex", gap: 8, marginBottom: 12 }, children: [_jsxs("div", { style: { flex: 1, display: "flex", alignItems: "center", gap: 8, background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 14, padding: "10px 12px" }, children: [_jsx(Plus, { size: 16, color: T.sub }), _jsx("input", { value: nuevo, onChange: e => setNuevo(e.target.value), onKeyDown: e => e.key === "Enter" && addExtra(), placeholder: "A\u00F1adir algo suelto (papel de cocina\u2026)", style: { border: "none", outline: "none", flex: 1, fontSize: 14, background: "transparent" } })] }), _jsx("button", { onClick: addExtra, style: { background: T.ink, color: "#fff", borderRadius: 14, padding: "0 16px", fontWeight: 700, fontSize: 14 }, children: "A\u00F1adir" })] }), _jsxs("div", { className: "yt-scroll", style: { flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14, paddingBottom: 8 }, children: [total === 0 && (_jsxs("div", { style: { textAlign: "center", color: T.sub, padding: "50px 24px" }, children: [_jsx("div", { style: { fontSize: 44, marginBottom: 10 }, children: "\uD83E\uDDFA" }), "La cesta est\u00E1 vac\u00EDa. Abre una receta y toca \u00ABA la compra\u00BB, o env\u00EDa tu semana planificada desde el plan."] })), Object.entries(porCat).map(([c, items]) => (_jsxs("section", { children: [_jsx("h2", { className: "yt-display", style: { fontSize: 13, fontWeight: 800, color: CAT[c].color, textTransform: "uppercase", letterSpacing: 0.6, margin: "0 0 6px" }, children: CAT[c].label }), _jsx("div", { style: { background: "#fff", borderRadius: 18, border: `1px solid ${T.line}`, overflow: "hidden" }, children: items.map((g, i) => {
                                    const k = "i:" + g.ingId, done = !!st.compra.checked[k], ing = ING_MAP[g.ingId];
                                    return (_jsxs("button", { onClick: () => toggle(k), style: { display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 13px", borderTop: i ? `1px solid ${T.line}` : "none", textAlign: "left" }, children: [_jsx("span", { style: { width: 22, height: 22, borderRadius: 8, border: `2px solid ${done ? T.ok : "#CFCABF"}`, background: done ? T.ok : "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }, children: done && _jsx(Check, { size: 14, color: "#fff", strokeWidth: 3 }) }), _jsx("span", { style: { fontSize: 20 }, children: ing.e }), _jsx("span", { style: { flex: 1, fontWeight: 600, fontSize: 14, textDecoration: done ? "line-through" : "none", color: done ? "#B7B2A9" : T.ink }, children: ing.n }), _jsx("span", { style: { fontSize: 12.5, color: T.sub, fontWeight: 600 }, children: g.texto })] }, g.ingId));
                                }) })] }, c))), st.compra.extras.length > 0 && (_jsxs("section", { children: [_jsx("h2", { className: "yt-display", style: { fontSize: 13, fontWeight: 800, color: T.sub, textTransform: "uppercase", letterSpacing: 0.6, margin: "0 0 6px" }, children: "Otros" }), _jsx("div", { style: { background: "#fff", borderRadius: 18, border: `1px solid ${T.line}`, overflow: "hidden" }, children: st.compra.extras.map((e, i) => {
                                    const k = "e:" + i + ":" + e, done = !!st.compra.checked[k];
                                    return (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10, padding: "10px 13px", borderTop: i ? `1px solid ${T.line}` : "none" }, children: [_jsx("button", { onClick: () => toggle(k), style: { width: 22, height: 22, borderRadius: 8, border: `2px solid ${done ? T.ok : "#CFCABF"}`, background: done ? T.ok : "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }, children: done && _jsx(Check, { size: 14, color: "#fff", strokeWidth: 3 }) }), _jsx("span", { style: { flex: 1, fontWeight: 600, fontSize: 14, textDecoration: done ? "line-through" : "none", color: done ? "#B7B2A9" : T.ink }, children: e }), _jsx("button", { "aria-label": `Eliminar ${e}`, onClick: () => quitarExtra(i), children: _jsx(Trash2, { size: 15, color: "#C9C4BB" }) })] }, k));
                                }) })] })), total > 0 && (_jsxs("div", { style: { display: "flex", gap: 8 }, children: [_jsx("button", { onClick: vaciarComprados, style: { flex: 1, background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 14, padding: 12, fontWeight: 700, fontSize: 13, color: T.sub }, children: "Quitar lo comprado" }), _jsx("button", { onClick: vaciarTodo, style: { flex: 1, background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 14, padding: 12, fontWeight: 700, fontSize: 13, color: T.bad }, children: "Vaciar lista" })] }))] })] }));
}
...
function Perfil({ st, setSt, editarIng }) {
    ...
    return (_jsxs("div", { className: "yt-scroll", style: { padding: "8px 20px 20px", height: "100%", overflowY: "auto" }, children: [_jsx("h1", { className: "yt-display", style: { fontSize: 26, fontWeight: 800, margin: "0 0 12px" }, children: "Tu cocina" }), _jsx("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 18 }, children: [
                    ...
                    [st.favs.length, "favoritas guardadas", "❤️"],
                ].map(([v, l, ic], i) => (_jsxs("div", { style: { background: "#fff", borderRadius: 18, border: `1px solid ${T.line}`, padding: "14px 14px" }, children: [_jsx("div", { style: { fontSize: 18 }, children: ic }), _jsx("div", { className: "yt-display", style: { fontSize: 24, fontWeight: 800, margin: "2px 0" }, children: v }), _jsx("div", { style: { fontSize: 12, color: T.sub, fontWeight: 600 }, children: l })] }, i))) }), kcalSemana > 0 && (_jsxs("div", { style: { background: "#fff", borderRadius: 18, border: `1px solid ${T.line}`, padding: 14, marginBottom: 18, display: "flex", alignItems: "center", gap: 10 }, children: [_jsx(Soup, { size: 20, color: T.brand }), _jsxs("div", { style: { fontSize: 13.5 }, children: [_jsxs("b", { children: [kcalSemana, " kcal"] }), " planificadas esta semana ", _jsx("span", { style: { color: T.sub }, children: "(por raci\u00F3n, seg\u00FAn tu plan)" })] })] })), _jsxs("section", { style: { marginBottom: 18 }, children: [_jsx("h2", { className: "yt-display", style: { fontSize: 16, fontWeight: 800, margin: "0 0 8px" }, children: "Logros" }), _jsx("div", { style: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }, children: logros.map(l => (_jsxs("div", { style: { background: l.ok ? "#fff" : "#F1EEE8", borderRadius: 16, border: `1.5px solid ${l.ok ? "#EED9B8" : T.line}`, padding: "11px 12px", opacity: l.ok ? 1 : 0.65 }, children: [_jsx("div", { style: { fontSize: 20, filter: l.ok ? "none" : "grayscale(1)" }, children: l.icon }), _jsx("div", { style: { fontWeight: 800, fontSize: 13, margin: "3px 0 1px" }, children: l.n }), _jsx("div", { style: { fontSize: 11.5, color: T.sub }, children: l.d })] }, l.id))) })] }), _jsxs("section", { style: { marginBottom: 18 }, children: [_jsx("h2", { className: "yt-display", style: { fontSize: 16, fontWeight: 800, margin: "0 0 8px" }, children: "Tus gustos" }), nVal === 0 ? (_jsx("p", { style: { color: T.sub, fontSize: 13.5, background: "#fff", borderRadius: 16, border: `1px solid ${T.line}`, padding: 14 }, children: "A\u00FAn no has catado nada. P\u00E1sate por la pesta\u00F1a Descubrir y desliza unas cuantas cartas." })) : (_jsxs(_Fragment, { children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 14, padding: "9px 12px", marginBottom: 8 }, children: [_jsx(Search, { size: 16, color: T.sub }), _jsx("input", { value: q, onChange: e => setQ(e.target.value), placeholder: "Buscar en lo catado\u2026", style: { border: "none", outline: "none", flex: 1, fontSize: 14, background: "transparent" } })] }), _jsx("div", { style: { background: "#fff", borderRadius: 18, border: `1px solid ${T.line}`, overflow: "hidden", maxHeight: 320, overflowY: "auto" }, className: "yt-scroll", children: valorados.map((i, idx) => {
                                    ...
                                    return (_jsxs("button", { onClick: () => editarIng(i), style: { display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "9px 12px", borderTop: idx ? `1px solid ${T.line}` : "none", textAlign: "left" }, children: [_jsx(IngImg, { ing: i, size: 36, round: 11 }), _jsx("span", { style: { flex: 1, fontWeight: 600, fontSize: 13.5 }, children: i.n }), _jsx(Chip, { color: lbl[1], bg: lbl[2], style: { fontSize: 11 }, children: lbl[0] })] }, i.id));
                                }) }), _jsx("p", { style: { fontSize: 12, color: T.sub, margin: "6px 2px 0" }, children: "Toca cualquiera para cambiar de opini\u00F3n o afinar matices." })] }))] }), _jsx("section", { style: { paddingBottom: 20 }, children: !confirmReset ? (_jsx("button", { onClick: () => setConfirmReset(true), style: { width: "100%", background: "#fff", border: `1.5px solid ${T.line}`, borderRadius: 14, padding: 13, fontWeight: 700, fontSize: 13.5, color: T.bad }, children: "Empezar de cero" })) : (_jsxs("div", { style: { background: "#FBE6EA", borderRadius: 16, padding: 14 }, children: [_jsx("p", { style: { margin: "0 0 10px", fontSize: 13.5, fontWeight: 600, color: "#9C2B3D" }, children: "Se borrar\u00E1n gustos, plan, compra, favoritas y probadas. \u00BFSeguro?" }), _jsxs("div", { style: { display: "flex", gap: 8 }, children: [_jsx("button", { onClick: () => { setSt(estadoInicial()); setConfirmReset(false); }, style: { flex: 1, background: T.bad, color: "#fff", borderRadius: 12, padding: 11, fontWeight: 700, fontSize: 13.5 }, children: "S\u00ED, borrar todo" }), _jsx("button", { onClick: () => setConfirmReset(false), style: { flex: 1, background: "#fff", borderRadius: 12, padding: 11, fontWeight: 700, fontSize: 13.5 }, children: "Cancelar" })] })] })) })] }));
}
/* ---------------- APP ---------------- */
...
export default function App() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const r = await __store.get("yantia:v1");
                if (r?.value) {
    ...
    useEffect(() => {
        if (!cargado)
            return;
        const t = setTimeout(() => { __store.set("yantia:v1", JSON.stringify(st)).catch(() => { }); }, 600);
        return () => clearTimeout(t);
    }, [st, cargado]);
    ...
    const addSemanaCompra = () => {
        ...
        setSt(s => {
            ...
            for (const id of Object.values(s.plan)) {
                ...
                if (r)
                    items.push(...r.ing.filter(it => !BASICOS.has(it.id)).map(it => ({ ingId: it.id, q: it.q, recetaId: r.id })));
            }
    ...
    if (!cargado) {
        return (_jsxs("div", { className: "yt-app", style: { height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: T.bg }, children: [_jsx("style", { children: GLOBAL_CSS }), _jsxs("div", { className: "yt-pop", style: { textAlign: "center" }, children: [_jsx("div", { style: { fontSize: 44 }, children: "\uD83C\uDF73" }), _jsx("div", { className: "yt-display", style: { fontWeight: 800, fontSize: 20, marginTop: 6 }, children: "Yantia" })] })] }));
    }
    return (_jsxs("div", { className: "yt-app", style: { height: "100vh", background: T.bg, display: "flex", justifyContent: "center" }, children: [_jsx("style", { children: GLOBAL_CSS }), _jsxs("div", { style: { width: "100%", maxWidth: 430, height: "100%", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }, children: [_jsxs("header", { style: { display: "flex", alignItems: "center", gap: 8, padding: "14px 20px 6px" }, children: [_jsx("div", { style: { width: 34, height: 34, borderRadius: 11, background: T.brand, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 10px rgba(232,83,44,0.35)" }, children: _jsx(Utensils, { size: 17, color: "#fff" }) }), _jsx("span", { className: "yt-display", style: { fontWeight: 800, fontSize: 17, letterSpacing: -0.3 }, children: "Yantia" })] }), _jsxs("main", { style: { flex: 1, minHeight: 0, position: "relative" }, children: [tab === "descubrir" && _jsx(Descubrir, { st: st, setSt: setSt, onOpenFormas: (ing, like) => setFormas({ ing, like }) }), tab === "recetas" && _jsx(Recetas, { st: st, setSt: setSt, abrir: setDetalle, sorprender: sorprender }), tab === "plan" && _jsx(Plan, { st: st, setSt: setSt, abrirReceta: setDetalle, addSemanaCompra: addSemanaCompra }), tab === "compra" && _jsx(Compra, { st: st, setSt: setSt }), tab === "perfil" && _jsx(Perfil, { st: st, setSt: setSt, editarIng: (ing) => setFormas({ ing: ing.f ? ing : { ...ing, f: ["tal cual"] }, like: st.prefs[ing.id]?.like ?? true, editar: true }) })] }), _jsx("nav", { style: { display: "flex", background: "#fff", borderTop: `1px solid ${T.line}`, padding: "8px 8px calc(10px + env(safe-area-inset-bottom))" }, children: TABS.map(({ id, l, I }) => {
                            const act = tab === id;
                            return (_jsxs("button", { onClick: () => { setTab(id); setDetalle(null); }, "aria-label": l, style: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "5px 0", color: act ? T.brand : "#A29D93" }, children: [_jsx(I, { size: 21, strokeWidth: act ? 2.4 : 2 }), _jsx("span", { style: { fontSize: 10.5, fontWeight: act ? 800 : 600 }, children: l })] }, id));
                        }) }), detalle && !cocina && (_jsx(RecetaDetalle, { recId: detalle, st: st, setSt: setSt, cerrar: () => setDetalle(null), cocinar: setCocina, addCompra: addCompra, addPlan: setPlanPicker })), cocina && _jsx(ModoCocina, { recId: cocina, cerrar: () => setCocina(null), marcarProbada: marcarProbada }), formas && !formas.editar && _jsx(FormasSheet, { data: formas, onClose: () => setFormas(null), onSave: guardarFormas }), formas && formas.editar && (_jsx(EditarGusto, { data: formas, st: st, onClose: () => setFormas(null), onSave: (ingId, like, sel) => { setSt(s => ({ ...s, prefs: { ...s.prefs, [ingId]: { like, formas: sel } } })); setFormas(null); } })), planPicker && (_jsxs(Sheet, { onClose: () => setPlanPicker(null), title: "\u00BFPara cu\u00E1ndo lo planificamos?", children: [_jsx("div", { style: { display: "flex", flexDirection: "column", gap: 8 }, children: DIAS.map((d, di) => (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [_jsx("span", { className: "yt-display", style: { width: 44, fontWeight: 800, fontSize: 14 }, children: d }), COMIDAS_PLAN.map(c => {
                                            const ocupado = st.plan[`${di}-${c.id}`];
                                            return (_jsxs("button", { onClick: () => colocarEnPlan(di, c.id), style: { flex: 1, padding: "10px 8px", borderRadius: 12, fontSize: 13, fontWeight: 700,
                                                    background: ocupado ? "#F1EEE8" : T.brandSoft, color: ocupado ? T.sub : T.brand,
                                                    border: `1.5px solid ${ocupado ? T.line : "#F5C9B8"}` }, children: [c.l, ocupado ? " ·↺" : ""] }, c.id));
                                        })] }, d))) }), _jsx("p", { style: { fontSize: 12, color: T.sub, margin: "10px 2px 0" }, children: "Los huecos con \u21BA ya tienen plato: si los eliges, se sustituye." })] })), toast && (_jsx("div", { className: "yt-up", role: "status", style: { position: "absolute", bottom: 86, left: 20, right: 20, background: T.ink, color: "#fff", borderRadius: 14, padding: "12px 16px", fontSize: 13.5, fontWeight: 600, textAlign: "center", boxShadow: "0 10px 26px rgba(0,0,0,0.3)", zIndex: 80 }, children: toast }))] })] }));
}
...
function EditarGusto({ data, st, onClose, onSave }) {
    ...
    const [sel, setSel] = useState(() => {
        const base = {};
        if (tieneFormas)
            for (const f of ing.f)
                base[f] = pref.formas?.[f] ?? (pref.like ?? true);
        return base;
    });
    useEffect(() => { if (tieneFormas)
        setSel(s => Object.fromEntries(Object.keys(s).map(f => [f, pref.formas?.[f] ?? like]))); }, [like]); // eslint-disable-line
    return (_jsxs(Sheet, { onClose: onClose, title: ing.n, children: [_jsxs("div", { style: { display: "flex", gap: 8, marginBottom: 14 }, children: [_jsx("button", { onClick: () => setLike(true), style: { flex: 1, padding: 13, borderRadius: 14, fontWeight: 800, fontSize: 14, background: like ? "#E4F5EC" : "#F6F3EE", color: like ? "#14603D" : T.sub, border: `1.5px solid ${like ? T.ok : T.line}` }, children: "\uD83D\uDC9A Me gusta" }), _jsx("button", { onClick: () => setLike(false), style: { flex: 1, padding: 13, borderRadius: 14, fontWeight: 800, fontSize: 14, background: !like ? "#FBE6EA" : "#F6F3EE", color: !like ? "#9C2B3D" : T.sub, border: `1.5px solid ${!like ? T.bad : T.line}` }, children: "\u270B No me va" })] }), tieneFormas && (_jsx("div", { style: { display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }, children: ing.f.map(f => (_jsxs("button", { onClick: () => setSel(s => ({ ...s, [f]: !s[f] })), style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 14px", borderRadius: 14, background: sel[f] ? "#E4F5EC" : "#F6F3EE", border: `1.5px solid ${sel[f] ? T.ok : T.line}`, fontSize: 14, fontWeight: 600, color: sel[f] ? "#14603D" : T.sub, textAlign: "left" }, children: [_jsx("span", { children: f }), sel[f] ? _jsx(Check, { size: 17, color: T.ok }) : _jsx(X, { size: 17, color: "#B7B2A9" })] }, f))) })), _jsx(BtnPrimario, { onClick: () => onSave(ing.id, like, tieneFormas ? sel : {}), children: "Guardar" })] }));
}
